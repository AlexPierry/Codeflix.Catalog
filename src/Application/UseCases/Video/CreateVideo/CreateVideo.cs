...
public class CreateVideo : ICreateVideo
{
    ...
    private async Task UploadVideos(CreateVideoInput input, Entities.Video video, CancellationToken cancellationToken)
    {
        if (input.Media is not null)
        {
            ...
            var mediaPath = await _storageService.Upload(fileName, input.Media.FileStream, input.Media.ContentType, cancellationToken);
        ...
        if (input.Trailer is not null)
        {
            ...
            var trailerPath = await _storageService.Upload(fileName, input.Trailer.FileStream, input.Trailer.ContentType, cancellationToken);
